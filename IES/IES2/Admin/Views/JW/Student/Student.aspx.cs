    ...
    public partial class Student : System.Web.UI.Page
    {
        ...
        private void DataBinder(int pageindex)
        {
            ...
            if (parms != "")
            {
                ...
                specialtyid = Convert.ToInt32(ary[10]);
                Session["Parms"] = parms;
            }

            IES.JW.Model.User _user = new IES.JW.Model.User { Key = key, Role = role, modevalue = modevalue, OrganizationID = organizationid, SpecialtyID = specialtyid, ClassID = classid, IsLocked = islocked, IsRegister = isregister, IsAssistant = isassistant, IsShow = isshow, IsInSchool = isinschool, EntryDate = entrydate };
            IES.G2S.JW.BLL.UserBLL userbll = new IES.G2S.JW.BLL.UserBLL();
            List<IES.JW.Model.User> userlist = userbll.Student_List(_user, pageindex, AspNetPager1.PageSize);
            if (userlist != null)
            if (userlist.Count > 0)
            {
        ...
        public void DataParm()
        {
            ...
            rptorg.DataBind();
            rptety.DataSource = parmlist.etylist;
            rptety.DataBind();
            rptsp.DataSource = parmlist.splist;
            rptsp.DataBind();
        }
        public void GetSession()
        {
            IES.JW.Model.User student = Session["Student"] as IES.JW.Model.User;
        ...
        protected void btnInfo_Click(object sender, EventArgs e)
        {
            ...
            IES.JW.Model.User _user = new IES.JW.Model.User { UserID = id };
            IES.G2S.JW.BLL.UserBLL userbll = new IES.G2S.JW.BLL.UserBLL();
            bool result = userbll.User_Del(_user);
            if (result == true)
            { DataBinder(1); }
        }
        //批量删除
