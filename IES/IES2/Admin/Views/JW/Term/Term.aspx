<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Term.aspx.cs" Inherits="Admin.Views.JW.Term.Term" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="sousuo_box">
        <div class="filter_item">
            <div style="float: left; margin-left: 20px">
                <asp:DropDownList ID="TermYear" Width="200px" Height="32px" runat="server"></asp:DropDownList>
            </div>
            <a class="add_student" href="javascript:ADD();" style="margin-right: 15px"><i class="icon_admin add_btn"></i>新增校历</a>
            <p class="search_btn">
                <input id="Key" runat="server" type="text" placeholder="输入学年、学期进行搜索"><a runat="server" onserverclick="btnSelect_Click" style="float: right; position: relative; margin-right: 3px; margin-top: -29px;" class="icon_admin search_icon"></a>
            </p>
        </div>
    </div>
                    ...
                    <tr class="item item_row" id='<%# Eval("TermID")%>'>
                        ...
                        <td>
                            <p class='operation_box'>
                                <i title='详细' id='<%#Eval("TermID")%>' onclick="showhide(this.id,this)" class='icon_construction suo_btn'></i>
                                <i title='编辑' pid='<%#Eval("TermID")%>' onclick='Edit(this)' class='icon_admin edit_btn'></i>
                                <i title='删除' id='<%#Eval("TermID")%>' onclick="Del(this.id)" class='icon_admin delete_btn'></i>
                            </p>
                        </td>
                    </tr>
                    <tr id="son<%#Eval("TermID")%>" style="display: none">
                        <td></td>
                        <td colspan="5">
                            <asp:Repeater runat="server" ID="rpquestionlist">
                                <ItemTemplate>
                                    <div style="width: 110px; float: left;text-align:center">
                                        <p><label><%# Eval("LessonName")%></label></p>
                                        
                                        <p><label><%# Eval("StartTime")%> -<%# Eval("StartTime")%></label></p>
                                    </div>
                                </ItemTemplate>
                            </asp:Repeater>
                        </td>
    ...
    <script>
        ...
        function showhide(id, obj) {
            ...
            if (temp == true) {
                jq.show();
                obj.className = 'icon_construction sou_btn';
            }
            else {
                jq.hide();
                obj.className = 'icon_construction suo_btn';
            }
        }
    </script>
</asp:Content>
