...
namespace IES.Common
{
    ...
    public class Thumbnail
    {
        ...
        private void CreateThumbnail(string sFileDstPath, double LimitW, double LimitH)
        {
            if (this.image != null)
            {
                System.Drawing.Image image = this.image.Clone() as System.Drawing.Bitmap;
                byte[] data;
                ...
                //image = System.Drawing.Image.FromStream(_stream);
                string dir = sFileDstPath.Substring(0, sFileDstPath.LastIndexOf('\\'));
                if (!System.IO.Directory.Exists(dir))
                {
                    System.IO.Directory.CreateDirectory(dir);
                }
                if (System.IO.File.Exists(sFileDstPath))
                {
                    try { System.IO.File.Delete(sFileDstPath); }
                    catch { }
                }
                using (System.IO.FileStream fs = new FileStream(sFileDstPath, FileMode.Create))
                {
        ...
        public static void MakeAllThumbnail(string path, string filename, out List<string> savefiles)
        {
            MakeAllThumbnail(path, filename, 1, out savefiles);
        }
        ...
        /// <param name="_type">0为头像， 1为照片</param>
        /// <param name="savefiles"></param>
        public static void MakeAllThumbnail(string path, string filename, int _type, out List<string> savefiles)
        {
